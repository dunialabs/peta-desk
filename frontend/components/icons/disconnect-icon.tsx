export function DisconnectIcon({ className = "", isHovered = false }: { className?: string; isHovered?: boolean }) {
  return (
    <div className="relative inline-flex items-center justify-center w-6 h-6">
      {isHovered && (
        <div className="absolute inset-0 rounded bg-black opacity-5 dark:bg-white dark:opacity-10" />
      )}
      <svg
        ...
        viewBox="0 0 16 16"
        fill="none"
        className={`relative z-10 ${isHovered ? 'text-red-500' : 'text-gray-900 dark:text-gray-100'} ${className}`}
      >
        <path
